...
@implementation YSFBadgeView

+ (instancetype)viewWithBadgeTip:(NSString *)badgeValue{
    if (!badgeValue) {
        badgeValue = @"";
    }
    YSFBadgeView *instance = [[YSFBadgeView alloc] init];
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    if (_badgeValue.integerValue > 9 || [_badgeValue isEqualToString:@"new"]) {
...
- (void)drawWithContent:(CGRect)rect context:(CGContextRef)context{
    ...
    if ([self badgeBackgroundColor]) {//外白色描边
        ...
        if ([self badgeValue].integerValue > 9 || [self.badgeValue isEqualToString:@"new"]) {
