 ...
 */
typedef NS_ENUM(NSUInteger, YSFTextAttachmentVerticalAlignment)
{
	/**
	 Baseline alignment (default)
	 */
	YSFTextAttachmentVerticalAlignmentBaseline = 0,
	
	/**
	 Align with top edge
	 */
	YSFTextAttachmentVerticalAlignmentTop,
	
	/**
	 Align with center
	 */
	YSFTextAttachmentVerticalAlignmentCenter,
	
	/**
	 Align with bottom edge
	 */
	YSFTextAttachmentVerticalAlignmentBottom
};

/**
 Methods to implement for attachments to support inline drawing.
 */
@protocol YSFTextAttachmentDrawing <NSObject>
 ...
 */
@protocol YSFTextAttachmentHTMLPersistence <NSObject>
 ...
 */
@interface YSFTextAttachment : NSObject <NSCoding>
{
	...
	YSFTextAttachmentVerticalAlignment _verticalAlignment; // alignment in relation to the baseline
}
...
/**
 Initialize and return a YSFTextAttachment with the specified YSFHTMLElement and options. Convenience initializer. 
	The element must have a valid tagName. The size of the returned text attachment is determined by the element, constrained by the option's key for YSFMaxImageSize. Any valid image resource included in the element (denoted by the method attributeForKey: "src") is loaded and determines the text attachment size if it was not known before. If a size is too large the image is downsampled with sizeThatFitsKeepingAspectRatio() which preserves the aspect ratio. 
 ...
 */
+ (YSFTextAttachment *)textAttachmentWithElement:(YSFHTMLElement *)element options:(NSDictionary *)options;

/**
 The designated initializer for members of the YSFTextAttachment class cluster. If you need additional setup for custom subclasses then you should override this initializer.
 @param element A YSFHTMLElement that must have a valid tag name and should have a size. Any element attributes are copied to the text attachment's elements.
 @param options An NSDictionary of options. Used to specify the max image size with the key YSFMaxImageSize.
 @returns Returns an initialized YSFTextAttachment built using the element and options parameters.  */
 ...
 */
@property (nonatomic, assign) YSFTextAttachmentVerticalAlignment verticalAlignment;
...
/**
 ...
 These registrations are permanent during the run time of your app. Custom attachment classes must implement the initWithElement:options: initializer and can implement the YSFTextAttachmentDrawing and/or YSFTextAttachmentHTMLPersistence protocols.
