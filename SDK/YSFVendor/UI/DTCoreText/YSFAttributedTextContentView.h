//
//  YSFAttributedTextContentView.h
//  YSFCoreText
//
...
@class YSFAttributedTextContentView;
@class YSFCoreTextLayoutFrame;
@class YSFTextBlock;
 ...
 */
extern NSString * const YSFAttributedTextContentViewDidFinishLayoutNotification;

/**
 Protocol to provide custom views for elements in an YSFAttributedTextContentView. Also the delegate gets notified once the text view has been drawn.
 */
 ...
 */
- (void)attributedTextContentView:(YSFAttributedTextContentView *)attributedTextContentView willDrawLayoutFrame:(YSFCoreTextLayoutFrame *)layoutFrame inContext:(CGContextRef)context;
 ...
 */
- (void)attributedTextContentView:(YSFAttributedTextContentView *)attributedTextContentView didDrawLayoutFrame:(YSFCoreTextLayoutFrame *)layoutFrame inContext:(CGContextRef)context;
 ...
 */
- (BOOL)attributedTextContentView:(YSFAttributedTextContentView *)attributedTextContentView shouldDrawBackgroundForTextBlock:(YSFTextBlock *)textBlock frame:(CGRect)frame context:(CGContextRef)context forLayoutFrame:(YSFCoreTextLayoutFrame *)layoutFrame;
...
/**
 ...
 @param attachment The <YSFTextAttachment> that this view should represent
 ...
 */
- (UIView *)attributedTextContentView:(YSFAttributedTextContentView *)attributedTextContentView viewForLink:(NSURL *)url identifier:(NSString *)identifier frame:(CGRect)frame;
 ...
 */
- (UIView *)attributedTextContentView:(YSFAttributedTextContentView *)attributedTextContentView viewForAttributedString:(NSAttributedString *)string frame:(CGRect)frame;

@end


enum {
	YSFAttributedTextContentViewRelayoutNever            = 0,
	YSFAttributedTextContentViewRelayoutOnWidthChanged   = 1 << 0,
	YSFAttributedTextContentViewRelayoutOnHeightChanged  = 1 << 1,
};
typedef NSUInteger YSFAttributedTextContentViewRelayoutMask;


/**
 Attributed Text Content Views display attributed strings generated by YSFHTMLAttributedStringBuilder. They can display images and hyperlinks inline or optionally place custom subviews (which get provided via the <delegate> in the appropriate places. By itself content views do not scroll, for that there is the `UIScrollView` subclass <YSFAttributedTextView>.
 ...
 YSFAttributedTextContentView is designed to be used as the content view inside a YSFAttributedTextView and thus sizes its intrinsicContentSize always to be the same as the width of the set frame. Use YSFAttributedLabel if you don't require scrolling behavior.
 */

@interface YSFAttributedTextContentView : UIView
{
	NSAttributedString *_attributedString;
	YSFCoreTextLayoutFrame *_layoutFrame;
...
/**
 ...
 This does a full layout pass that is cached in <YSFCoreTextLayouter>. If you specify a frame that fits the result from this method then the resulting layoutFrame is reused.
 ...
 */
@property (atomic, strong) YSFCoreTextLayouter *layouter;


/**
 The layout frame to use for the receiver. Created by default.
 
 A layout frame is basically one rectangle, inset by the <edgeInsets>. By default this is automatically generated for the current <attributedString>. You can also create a <YSFCoreTextLayoutFrame> seperately and set this property to display the layout frame. This is usedful for example if you layout entire e-book and then set the <layoutFrame> for displaying individual pages.
 */
@property (atomic, strong) YSFCoreTextLayoutFrame *layoutFrame;
...
/**
 The delegate that is in charge of supplying custom behavior for the receiver. It must conform to <YSFAttributedTextContentViewDelegate> and provide custom subviews, link buttons, etc.
 */

@property (nonatomic, YSF_WEAK_PROPERTY) IBOutlet id <YSFAttributedTextContentViewDelegate> delegate;
...
/**
 ...
 If set to `NO` then your custom subview/button for hyperlinks is responsible for displaying hyperlinks. You can use <YSFLinkButton> to have links show a differently for normal and highlighted style
 */
...
/**
 ...
 If set to `YES` then all custom subviews will always be layouted. Set to `NO` to only layout visible subviews, e.g. in a scroll view. Defaults to `YES` if used stand-alone, `NO` inside a <YSFAttributedTextView>.
 */
...
/**
 ...
 When the view’s bounds change, that view automatically re-layouts its text according to the relayout mask. You specify the value of this mask by combining the constants described in YSFAttributedTextContentViewRelayoutMask using the C bitwise OR operator. Combining these constants lets you specify which dimensions will cause a re-layout if modified. The default value of this property is YSFAttributedTextContentViewRelayoutOnWidthChanged, which indicates that the text will be re-layouted if the width changes, but not if the height changes.
 */
@property (nonatomic) YSFAttributedTextContentViewRelayoutMask relayoutMask;

@end


/**
 You can globally customize the layer class to be used for new instances of <YSFAttributedTextContentView>. By itself it makes most sense to go with the default `CALayer`. For larger bodies of text, i.e. if there is scrolling then you should use a `CATiledLayer` subclass instead.
 */
@interface YSFAttributedTextContentView (Tiling)
 ...
 */
@interface YSFAttributedTextContentView (Drawing)
...
/**
 ...
 @see [YSFCoreTextLayoutFrame drawInContext:options:] for a list of available drawing options
 @returns A `UIImage` with the specified content
 */
- (UIImage *)contentImageWithBounds:(CGRect)bounds options:(YSFCoreTextLayoutFrameDrawingOptions)options;
 ...
 */
@interface YSFAttributedTextContentView (Cursor)
