...
extern const NSTimeInterval kYSFAnimatedImageDelayTimeIntervalMinimum;
...
//
@interface YSFAnimatedImage : NSObject
...
@interface YSFAnimatedImage (Logging)

+ (void)setLogBlock:(void (^)(NSString *logString, YSFLogLevel logLevel))logBlock logLevel:(YSFLogLevel)logLevel;
+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(YSFLogLevel)level;

@end

#define YSFLog(logLevel, format, ...) [YSFAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface YSFWeakProxy : NSProxy
