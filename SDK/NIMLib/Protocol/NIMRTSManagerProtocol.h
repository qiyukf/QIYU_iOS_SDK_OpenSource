...
@class YSF_NIMRTSOption;
@class YSF_NIMRTSRecordingInfo;
 ...
 */
typedef void(^YSF_NIMRTSRequestHandler)(NSError *error, NSString *sessionID);
 ...
 */
typedef void(^YSF_NIMRTSResponseHandler)(NSError *error, NSString *sessionID);


/**
 *  实时会话服务
 */
typedef NS_ENUM(NSInteger, YSF_NIMRTSService){
    /**
     *  包含可靠数据传输服务
     */
    YSF_NIMRTSServiceReliableTransfer    = 1,
    /**
     *  包含实时语音沟通服务
     */
    YSF_NIMRTSServiceAudio               = 4,
};

/**
 *  实时会话状态
 */
typedef NS_ENUM(NSInteger, YSF_NIMRTSStatus){
    /**
     *  已连接
     */
    YSF_NIMRTSStatusConnect,
    /**
     *  已断开
     */
    YSF_NIMRTSStatusDisconnect,
};

/**
 *  实时会话相关回调
 */
@protocol YSF_NIMRTSManagerDelegate <NSObject>
 ...
 *  @param caller 主叫帐号
 *  @param types 服务类型, YSF_NIMRTSService的组合
...
- (void)onRTS:(NSString *)sessionID
      service:(YSF_NIMRTSService)type
       status:(YSF_NIMRTSStatus)status
        error:(NSError *)error;
...
- (void)onRTSReceive:(NSString *)sessionID
                data:(NSData *)data
                from:(NSString *)user
              withIn:(YSF_NIMRTSService)channel;
 ...
 *  @param sessionID   实时会话ID
 */
- (void)onRTSRecordingInfo:(YSF_NIMRTSRecordingInfo *)info
                forSession:(NSString *)sessionID;
 ...
 *  实时会话协议
 */
@protocol YSF_NIMRTSManager <NSObject>

/**
 *  主叫发起实时会话请求
 *
 *  @param callees 被叫帐号列表, 当前版本只支持一个被叫
 *  @param types  实时会话服务类型, YSF_NIMRTSService的组合, 如果要同时发起可靠传输通道和音频通话, 使用 NIMRTSServiceReliableTransfer | NIMRTSServiceAudio
...
- (NSString *)requestRTS:(NSArray *)callees
                services:(NSUInteger)types
                  option:(YSF_NIMRTSOption *)option
              completion:(YSF_NIMRTSRequestHandler)completion;
...
- (void)responseRTS:(NSString *)sessionID
             accept:(BOOL)accept
             option:(YSF_NIMRTSOption *)option
         completion:(YSF_NIMRTSResponseHandler)completion;
...
- (BOOL)sendRTSData:(NSData *)data
               from:(NSString *)sessionID
                 to:(NSString *)userID
               with:(YSF_NIMRTSService)service;
 ...
 */
- (void)addDelegate:(id<YSF_NIMRTSManagerDelegate>)delegate;
 ...
 *  @param delegate 实时会话委托
 */
- (void)removeDelegate:(id<YSF_NIMRTSManagerDelegate>)delegate;
@end
