 ...
 */
typedef void(^YSF_NIMTeamHandler)(NSError *error);
 ...
 *  @param teamId 群组ID
 */
typedef void(^YSF_NIMTeamCreateHandler)(NSError *error,NSString *teamId);

/**
 *  群成员block
 *
 *  @param error   错误,如果成功则error为nil
 *  @param members 群成员列表,内部为YSF_NIMTeamMember
 */
typedef void(^YSF_NIMTeamMemberHandler)(NSError *error,NSArray *members);
 ...
 *  @param team  群信息
 */
typedef void(^YSF_NIMTeamFetchInfoHandler)(NSError *error,YSF_NIMTeam *team);
 ...
 */
typedef void(^YSF_NIMTeamApplyHandler)(NSError *error,YSF_NIMTeamApplyStatus applyStatus);


/**
 *  群组委托
 */
@protocol YSF_NIMTeamManagerDelegate <NSObject>
 ...
 *  @param team 添加的群组
 */
- (void)onTeamAdded:(YSF_NIMTeam *)team;
 ...
 *  @param team 更新的群组
 */
- (void)onTeamUpdated:(YSF_NIMTeam *)team;
 ...
 *  @param team 被移除的群组
 */
- (void)onTeamRemoved:(YSF_NIMTeam *)team;
 ...
 *  @param team 变动的群组
 */
- (void)onTeamMemberChanged:(YSF_NIMTeam *)team;
 ...
 *  @return 群组信息
 */
- (YSF_NIMTeam *)teamById:(NSString *)teamId;
 ...
 */
- (void)createTeam:(YSF_NIMCreateTeamOption *)option
              users:(NSArray *)users
         completion:(YSF_NIMTeamCreateHandler)block;
...
- (void)dismissTeam:(NSString*)teamId
         completion:(YSF_NIMTeamHandler)block;
...
- (void)addUsers:(NSArray  *)users
      ...
      completion:(YSF_NIMTeamMemberHandler)block;
 ...
 *  @param block    完成后的block回调
 */
- (void)updateTeamJoinMode:(YSF_NIMTeamJoinMode)joinMode
                    teamId:(NSString *)teamId
                completion:(YSF_NIMTeamHandler)block;
 ...
 *  @param teamId 群组ID
 *  @param block  完成后的block回调
 *  @discussion   这个接口可以一次性修改群的多个属性,如名称,公告等,传入的数据键值对是 {@(YSF_NIMTeamUpdateTag) : NSString},无效数据将被过滤
 */
...
- (void)applyToTeam:(NSString *)teamId
         ...
         completion:(YSF_NIMTeamApplyHandler)block;
...
- (void)fetchTeamInfo:(NSString *)teamId
           completion:(YSF_NIMTeamFetchInfoHandler)block;
 ...
 *  @param teamId 群组ID
 *  @return       返回成员信息
 *  @discussion   这个值永远不会返回nil,如果传入的userId和teamId是无效值或者本地还没有缓存信息,将返回只带有userId和teamId信息的YSF_NIMTeamMember
 */
- (YSF_NIMTeamMember *)teamMember:(NSString *)userId
                       inTeam:(NSString *)teamId;
 ...
 *  @param delegate 群组委托
 */
- (void)addDelegate:(id<YSF_NIMTeamManagerDelegate>)delegate;
 ...
 *  @param delegate 群组委托
 */
- (void)removeDelegate:(id<YSF_NIMTeamManagerDelegate>)delegate;
@end
