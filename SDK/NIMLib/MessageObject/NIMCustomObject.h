...
/*
 ...
 由于 SDK 并不能预测上层 APP 的应用场景，所以 YSF_NIMCustomObject 采取消息透传的方式以提供给上层开发者最大的自由度
 即 SDK 只负责发送和收取由 YSF_NIMCustomObject 中 id<YSF_NIMCustomAttachment> attachment 序列化和反序列化后的字节流
...
服务器配置:
 ...
 1.- (BOOL)messageHistoryEnabled  是否支持在消息历史中拉取当前这条消息,默认为YES。正常而言所有消息都会出现在通过 YSF_NIMConversationManager 调用(fetchMessageHistory:option:result:)返回的结果中，但是可以通过设置这个值来使得消息不出现在这其中。
 ...
 *  自定义消息对象附件序列化协议
 */
@protocol YSF_NIMCustomAttachmentCoding<NSObject>
@required
 ...
 *  用户自定义消息对象
 */
@interface YSF_NIMCustomObject : NSObject<YSF_NIMMessageObject>

/**
 *  用户自定义附件
 *  @discussion SDK负责将attachment通过encodeAttachment接口序列化后的结果进行透传
 */
@property(nonatomic, strong) id<YSF_NIMCustomAttachment>  attachment;
 ...
 */
+ (void)registerCustomDecoder:(id<YSF_NIMCustomAttachmentCoding>)decoder;
